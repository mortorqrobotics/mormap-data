...
const searchMaps = coroutine(function* (query) {
  ...
  let file = getPath(
    "cache/google/" + query.replace(/\s+/g, "+").replace(/\//g, "-")
  );
  let result;
  let cached = yield checkCache(
    file,
    coroutine(function* () {
      let url =
        "https://maps.googleapis.com/maps/api/place/textsearch/json?key=" +
        config.googlePlacesApiWebServiceKey +
        "&query=" +
        query;
...
const searchTba = coroutine(function* (path) {
  ...
  let cache = yield checkCache(
    file,
    coroutine(function* () {
      return yield request({
        uri: "http://www.thebluealliance.com/api/v3" + path,
        headers: { "X-TBA-Auth-Key": config.TBAKey },
      });
    })
  );
...
coroutine(function* () {
  ...
  let allTeams = {};
  let teamss = yield Promise.all(
    events.map((event) => searchTba("/event/" + event.key + "/teams"))
  );
    ...
    for (let team of teams) {
      if (
        ...
        allTeams[team.team_number] = {
          ...
          location: `${team.school_name}, ${team.city}, ${team.country}`,
        };
      }
    }
  }
  ...
  for (let teamNum of teamNums) {
    let query = allTeams[teamNum].location;
